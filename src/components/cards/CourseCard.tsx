...
export function CourseCard({ course, index = 0 }: { course: Course; index?: number }) {
  return <Link to={`/courses/${course.slug}`} className="focus-ring card-lift group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-ink/10 bg-white">
    <div className="relative aspect-[4/3] overflow-hidden"><img className="image-cover" src={course.image} alt={`Lớp ${course.name} tại Jody Music`} loading={index > 1 ? 'lazy' : undefined} />{course.badge && <span className="absolute left-4 top-4 rounded-full bg-lime px-3 py-2 text-[.63rem] font-bold uppercase tracking-wider text-ink">{course.badge}</span>}<span className="absolute right-4 top-4 grid h-11 w-11 place-items-center rounded-full bg-white text-ink transition-transform group-hover:rotate-[-12deg]"><ArrowRight size={18} /></span></div>
